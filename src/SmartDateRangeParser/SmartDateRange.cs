...
public readonly struct SmartDateRange(DateTime start, DateTime end)
{
    ...
    private static SmartDateRange ParseEnglish(string input)
    {
        return input switch
        {
            ...
            _ when input.StartsWith("last ") && input.IndexOf("business day", StringComparison.OrdinalIgnoreCase) >= 0
                => ParseLastBusinessDays(input),

            _ when input.StartsWith("last ") && input.IndexOf("day", StringComparison.OrdinalIgnoreCase) >= 0
